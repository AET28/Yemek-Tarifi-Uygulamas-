"""Ana Pencere - Mutfak Defteri editorial sidebar."""
...
from frontend.views.tarifler import TariflerSayfasi
from frontend.views.tarif_detay import TarifDetaySayfasi
...
class AnaPencere(QMainWindow):
    def __init__(self, vy: VeriYoneticisi, aktif_kullanici=None):
        ...
        self.setWindowTitle("Mutfak Defteri — Tarif Koleksiyonu")
    ...
    def _arayuz_olustur(self):
        ...
        icerik_sarici.setStyleSheet("background-color: #faf6f1;")
        ...
        self.sayfa_tarifler = TariflerSayfasi(self.vy)
        self.sayfa_tarif_detay = TarifDetaySayfasi(self.vy)
        self.sayfa_kullanicilar = KullanicilarSayfasi(self.vy)
        self.sayfa_raporlar = RaporlarSayfasi(self.vy)

        # Sinyaller
        self.sayfa_dashboard.tarif_secildi.connect(self._tarif_detay_ac)
        self.sayfa_tarifler.tarif_secildi.connect(self._tarif_detay_ac)
        self.sayfa_tarifler.veri_degisti.connect(self._tumunu_yenile)
        self.sayfa_tarif_detay.geri_istendi.connect(self._tarifler_sayfasina_don)
        self.sayfa_tarif_detay.veri_degisti.connect(self._tumunu_yenile)
        self.sayfa_kullanicilar.veri_degisti.connect(self._tumunu_yenile)

        self.yigin.addWidget(self.sayfa_dashboard)   # 0
        self.yigin.addWidget(self.sayfa_tarifler)    # 1
        self.yigin.addWidget(self.sayfa_tarif_detay) # 2
        self.yigin.addWidget(self.sayfa_kullanicilar) # 3
        self.yigin.addWidget(self.sayfa_raporlar)    # 4
    ...
    def _sidebar_olustur(self) -> QFrame:
        ...
        self._menu_butonu_ekle(layout, "Tarifler", 1)
        self._menu_butonu_ekle(layout, "Kullanıcılar", 3)
        ...
        cikis_btn.setStyleSheet(
            ...
            "color: #7d7369; border: 1px solid #d1c9c0; "
            "border-radius: 0; font-size: 14px; padding: 0; "
            " } "
            "QPushButton:hover { color: #faf6f1; "
    ...
    def _tarif_detay_ac(self, tarif_id: int):
        self.sayfa_tarif_detay.tarif_goster(tarif_id)
        self.yigin.setCurrentIndex(2)

    def _tarifler_sayfasina_don(self):
        self.sayfa_tarifler.yenile()
        self.yigin.setCurrentIndex(1)
        # Sidebar'da Tarifler butonunu seçili yap
        btn = self.buton_grubu.button(1)
        if btn:
            btn.setChecked(True)
